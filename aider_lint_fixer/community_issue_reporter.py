...
class CommunityIssueReporter:
    ...
    def analyze_for_community_issues(self, manual_attempts: List[ManualFixAttempt]) -> List[CommunityIssue]:
    ...
    def _create_community_issue(self, pattern: str, all_attempts: List[ManualFixAttempt], 
                               successful_attempts: List[ManualFixAttempt]) -> CommunityIssue:
    ...
    def _generate_issue_body(self, linter: str, rule_id: str, all_attempts: List[ManualFixAttempt],
                           successful_attempts: List[ManualFixAttempt]) -> str:
        """Generate the GitHub issue body."""
        success_rate = len(successful_attempts) / len(all_attempts) * 100
        avg_confidence = sum(a.user_confidence for a in successful_attempts) / len(successful_attempts)
...
def integrate_community_issue_reporting(community_learning, manual_attempts: List[ManualFixAttempt]) -> None:
    ...
    if potential_issues:
        ...
        if click.confirm("   Would you like to help improve the system by creating community issues?"):
            reporter.prompt_for_issue_creation()
    else:
        print(f"   No patterns identified for community contribution yet")
        print(f"   (Need {reporter.MIN_SAMPLES_FOR_ISSUE}+ successful fixes with {reporter.MIN_SUCCESS_RATE*100}%+ success rate)")
