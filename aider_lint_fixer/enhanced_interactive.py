...
def enhanced_interactive_mode(
    ...
    if enable_community_learning:
        print(f"   {Fore.GREEN}🌍 Community learning enabled - your choices help improve the system{Style.RESET_ALL}")
    print()
...
def _process_fixable_error(
    error_analysis: ErrorAnalysis, 
    index: int, 
    enable_community_learning: bool
) -> InteractiveChoice:
...
def _process_unfixable_error(
    ...
    if error_analysis.complexity == FixComplexity.MANUAL:
        ...
        print(f"   {Fore.YELLOW}⚠️  WARNING: This error is complex and may need domain knowledge{Style.RESET_ALL}")
    
    print(f"   {Fore.CYAN}💡 Reason not fixable: {_get_unfixable_reason(error_analysis)}{Style.RESET_ALL}")
...
class CommunityLearningIntegration:
    ...
    def record_interactive_choices(self, choices: List[InteractiveChoice]) -> None:
        ...
        for choice in choices:
            ...
            print(f"   📝 Recorded choice: {choice.choice.value} for {choice.error_analysis.error.rule_id}")
