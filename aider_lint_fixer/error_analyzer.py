...
class ErrorAnalyzer:
    ...
    RULE_CATEGORIES = {
        ...
        'pylint': {
            ...
            ErrorCategory.IMPORT: ['import-error', 'no-name-in-module', 'relative-beyond-top-level'],
        ...
        'ansible-lint': {
            ErrorCategory.SYNTAX: [
                'yaml', 'syntax', 'parse', 'invalid', 'malformed'
            ],
            ErrorCategory.STYLE: [
                'name', 'description', 'meta', 'tags', 'formatting'
            ],
            ErrorCategory.SECURITY: [
                'risky-file-permissions', 'risky-shell-pipe', 'command-instead-of-shell',
                'no-changed-when', 'no-handler', 'deprecated'
            ],
            ErrorCategory.LOGIC: [
                'always-run', 'become-user-without-become', 'empty-string-compare',
                'literal-compare', 'no-jinja-when', 'octal-values'
            ],
            ErrorCategory.IMPORT: [
                'role-name', 'galaxy', 'requirements'
            ]
        }
    }
    ...
    def _is_fixable(self, error: LintError, category: ErrorCategory, complexity: FixComplexity) -> bool:
    ...
    def get_prioritized_errors(self, file_analyses: Dict[str, FileAnalysis], 
                             max_errors: Optional[int] = None) -> List[ErrorAnalysis]:
        ...
        all_analyses.sort(
            key=lambda x: (-x.priority, x.complexity.value, x.error.line)
        )
