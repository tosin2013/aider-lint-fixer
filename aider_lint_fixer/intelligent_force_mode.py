...
class IntelligentForceMode:
    ...
    def _extract_error_features(self, error_analysis: ErrorAnalysis) -> np.ndarray:
        ...
        features.extend([
            1 if error.rule_id == 'no-undef' else 0,  # Dangerous undefined variable
        ...
        # Context features
        features.extend([
            error_analysis.complexity.value if hasattr(error_analysis.complexity, 'value') else 1,
    ...
    def _get_base_confidence(self, error_analysis: ErrorAnalysis) -> float:
        ...
        dangerous_rules = ['no-undef', 'no-global-assign', 'no-implicit-globals']
    ...
    def _build_dependency_graph(self, error_analyses: List[ErrorAnalysis]):
        """Build code dependency graph for cascade prediction."""
        # Simple implementation - in production would use AST analysis
        ...
        # Add nodes and edges based on file relationships
    ...
    def _predict_cascades(self, force_decisions: List[ForceDecision]):
        ...
        for decision in force_decisions:
            if decision.action in ['auto_force', 'batch_confirm']:
                ...
                # Find connected files in dependency graph
                if file_path in self.dependency_graph:
                    connected_files = list(self.dependency_graph.neighbors(file_path))
                    decision.predicted_cascades = connected_files[:3]  # Limit to top 3
    
    def _optimize_batching(self, force_decisions: List[ForceDecision], is_chaotic: bool) -> List[BatchPlan]:
    ...
    def _create_optimal_batches(self, decisions: List[ForceDecision], is_chaotic: bool) -> List[BatchPlan]:
        ...
        for batch_id in range(n_batches):
            batch_decisions = [decisions[i] for i, label in enumerate(cluster_labels) if label == batch_id]
            
            if batch_decisions:
                avg_confidence = np.mean([d.confidence for d in batch_decisions])
                total_risk_factors = sum(len(d.risk_factors) if d.risk_factors else 0 for d in batch_decisions)
    ...
    def _create_execution_strategy(self, force_decisions: List[ForceDecision], 
                                 batch_plans: List[BatchPlan], is_chaotic: bool) -> Dict:
    ...
    def _generate_recommendations(self, force_decisions: List[ForceDecision], is_chaotic: bool) -> List[str]:
        """Generate intelligent recommendations for the user."""
        recommendations = []
        
        auto_force_count = sum(1 for d in force_decisions if d.action == 'auto_force')
        dangerous_count = sum(1 for d in force_decisions 
                            if d.error_analysis.error.rule_id == 'no-undef')
        
        if is_chaotic:
            recommendations.append(f"🏥 CHAOTIC CODEBASE DETECTED: {len(force_decisions)} errors found")
            recommendations.append(f"🤖 {auto_force_count} safe errors will be auto-fixed without confirmation")
            recommendations.append(f"⚠️  {dangerous_count} dangerous errors require manual review")
            recommendations.append(f"📊 Use progressive fixing: safe errors first, then risky ones")
        
        if auto_force_count > 50:
            recommendations.append(f"⚡ Large auto-force batch: Consider running in smaller chunks")
        
        if dangerous_count > 10:
            recommendations.append(f"🏗️  Many undefined variables: Consider architect mode for expert review")
        
        return recommendations
    
    def learn_from_outcome(self, decision: ForceDecision, success: bool, 
                          created_new_errors: bool = False):
        """Learn from fix outcomes to improve future predictions."""
        outcome = {
            'decision': decision,
            'success': success,
            'created_new_errors': created_new_errors,
            'confidence_was_correct': (success and decision.confidence > 0.7) or 
                                    (not success and decision.confidence < 0.7)
        }
    ...
    def _update_confidence_thresholds(self):
        ...
        high_conf_success = sum(1 for o in recent_outcomes 
                              if o['decision'].confidence > 0.8 and o['success'])
        high_conf_total = sum(1 for o in recent_outcomes 
                            if o['decision'].confidence > 0.8)
