...
class IntelligentForceMode:
    ...
    def _build_dependency_graph(self, error_analyses: List[ErrorAnalysis]):
        ...
        try:
            ...
            for file_path in files_with_errors:
                self.dependency_graph.add_node(
                    file_path, type="file", error_count=len(files_with_errors[file_path])
                )
    ...
    def _predict_cascades(self, force_decisions: List[ForceDecision]):
        ...
        for decision in force_decisions:
            if decision.action in ["auto_force", "batch_confirm"]:
                ...
                if error_type in ["no-undef", "no-global-assign"]:
    ...
    def _calculate_cascade_risk(self, error_type: str, edge_data: dict) -> float:
        ...
        error_risk_multipliers = {
            "no-undef": 0.8,  # Undefined variables have high cascade risk
    ...
    def _create_execution_strategy(
        self, force_decisions: List[ForceDecision], batch_plans: List[BatchPlan], is_chaotic: bool
    ) -> Dict:
    ...
    def _generate_recommendations(
        ...
        dangerous_count = sum(
            1 for d in force_decisions if d.error_analysis.error.rule_id == "no-undef"
        )
        ...
        if dangerous_count > 10:
            recommendations.append(
                f"🏗️  Many undefined variables: Consider architect mode for expert review"
            )
        ...
        if dep_insights:
            if dep_insights.get("highly_connected_files"):
                ...
                recommendations.append(
                    ...
                    f"fixes may have cascading effects"
                )
            ...
            if dep_insights.get("import_heavy_files"):
                ...
                recommendations.append(
                    ...
                    f"potential refactoring candidates"
                )
