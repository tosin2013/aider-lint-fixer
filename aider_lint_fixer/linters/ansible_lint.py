...
class AnsibleLintLinter(BaseLinter):
    ...
    def _parse_human_readable_output(self, stdout: str, stderr: str) -> Tuple[List[LintError], List[LintError]]:
    ...
    def run_with_profile(self, profile: str = 'basic', file_paths: Optional[List[str]] = None) -> LinterResult:
        """Run ansible-lint with a specific profile."""
        if profile not in self.PROFILES:
            raise ValueError(f"Unsupported profile: {profile}. Supported: {list(self.PROFILES.keys())}")
