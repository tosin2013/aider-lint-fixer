...
class RuleScraper:
    ...
    def _is_yaml_rule_fixable(self, rule_name: str, description: str) -> bool:
        ...
        fixable_yaml_rules = [
            "line-length", "trailing-spaces", "indentation", "comments",
            "document-start", "empty-lines", "new-line-at-end-of-file",
            "brackets", "colons", "commas", "braces"
        ]
        ...
        for link in soup.find_all('a', href=True):
            ...
            if '/rules/' in href and not href.endswith('/rules/'):
                ...
                if rule_id and not any(x in rule_id for x in ['#', '?', 'deprecated', 'removed']):
                    ...
                    if link.parent:
                        parent_text = link.parent.get_text() if hasattr(link.parent, 'get_text') else str(link.parent)
                        # Only consider it fixable if the wrench emoji is in the same line/element as the rule
                        if '🔧' in parent_text and len(parent_text) < 300:  # Reasonable length limit
                            auto_fixable = True
    ...
    def _is_ansible_rule_fixable(self, rule_id: str, description: str) -> bool:
        ...
        fixable_patterns = [
            "line-length", "spacing", "comments", "document-start",
            "trailing", "whitespace", "indent"
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--linter", choices=["ansible-lint", "eslint", "flake8", "all"],
                       default="all", help="Which linter to scrape")
