...
def scraped_rules():
    ...
    if not scraped_file.exists():
        pytest.skip("Scraped rules file not found. Run: python aider_lint_fixer/rule_scraper.py --linter ansible-lint --output scraped_ansible_rules.json")
...
def generate_yaml_test_cases(rule_id, expected_fixable):
    ...
    if "line-length" in rule_id:
        test_cases.extend([
            ...
            ("yaml[line-length] Line exceeds maximum", "ansible-lint", rule_id, expected_fixable),
        ])
        ...
        test_cases.extend([
            ...
            ("yaml[comments] Comment formatting issue", "ansible-lint", rule_id, expected_fixable),
        ])
        ...
        test_cases.extend([
            ...
            ("yaml[document-start] Document start issue", "ansible-lint", rule_id, expected_fixable),
        ])
    elif "trailing-spaces" in rule_id:
        test_cases.extend([
            ("Spaces are found at the end of lines", "ansible-lint", rule_id, expected_fixable),
            ("yaml[trailing-spaces] Trailing whitespace", "ansible-lint", rule_id, expected_fixable),
        ])
    elif "indentation" in rule_id:
        test_cases.extend([
            ("Wrong indentation: expected 4 but found 2", "ansible-lint", rule_id, expected_fixable),
            ("yaml[indentation] Indentation error", "ansible-lint", rule_id, expected_fixable),
        ])
    else:
        # Generic YAML test case
        test_cases.append((f"{rule_id} formatting error", "ansible-lint", rule_id, expected_fixable))
    ...
    for rule_id, rule_info in ansible_rules.items():
        ...
        if rule_id.startswith("yaml["):
            ...
            test_cases.extend([
                ...
                ("jinja[spacing] Template spacing issue", "ansible-lint", rule_id, expected_fixable),
            ])
        else:
            # General test case
            test_cases.append((description or f"{rule_id} error", "ansible-lint", rule_id, expected_fixable))
...
class TestEnhancedPatternMatching:
    ...
    def test_yaml_rule_classification(self, smart_classifier, scraped_rules):
        ...
        yaml_test_cases = [
            ...
            ("Wrong indentation: expected 4 but found 2", "ansible-lint", "yaml[indentation]", True),
        ]
    ...
    def test_comprehensive_classification_accuracy(self, smart_classifier, scraped_rules):
        ...
        print(f"   High confidence: {high_confidence_count}/{len(valid_test_cases)} = {high_confidence_rate:.1f}%")
        
        # Should achieve high accuracy with scraped rules
        assert accuracy >= 80, f"Overall accuracy should be ≥80%, got {accuracy:.1f}%"
        assert high_confidence_rate >= 70, f"High confidence rate should be ≥70%, got {high_confidence_rate:.1f}%"
    ...
    def test_performance_benchmark(self, smart_classifier, scraped_rules):
        ...
        valid_test_cases = [
            case for case in test_cases 
            if not case[2].startswith(('javascript:', '.', '#'))
        ][:50]  # Limit for performance testing
    ...
    def test_method_distribution(self, smart_classifier, scraped_rules):
        ...
        assert rule_knowledge_rate >= 50, f"Rule knowledge usage should be ≥50%, got {rule_knowledge_rate:.1f}%"
...
class TestMLLearningSystem:
    ...
    def test_ml_classification_improvement(self, smart_classifier):
        ...
        print(f"\n🎯 ML Learning Results:")
        print(f"   Initial: fixable={initial_result.fixable}, confidence={initial_confidence:.2f}, method={initial_result.method}")
        print(f"   Trained: fixable={trained_result.fixable}, confidence={trained_result.confidence:.2f}, method={trained_result.method}")

        # After training, the system should have learned
        # Note: The improvement might be subtle depending on the ML implementation
        assert trained_result.confidence >= initial_confidence * 0.8, "Confidence should not decrease significantly"
