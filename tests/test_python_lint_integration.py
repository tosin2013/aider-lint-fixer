...
'''
    ...
    def _test_modular_pylint(self):
        ...
        print(f"✅ Modular pylint: {len(result.errors)} errors, {len(result.warnings)} warnings detected")
    ...
    def _test_legacy_pylint(self):
        ...
        print(f"✅ Legacy pylint: {len(result.errors)} errors, {len(result.warnings)} warnings detected")
        print(f"   Raw output preview: {result.raw_output[:100]}")
...
class TestPythonLintCLIIntegration(unittest.TestCase):
    ...
    def test_cli_with_python_linters(self):
        ...
        if 'python' in project_info.languages:
            ...
            self.assertTrue(python_linters_available, "At least one Python linter should be available")
